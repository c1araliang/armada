...
* **Loadings** for both components are reported; substantive interpretation comes from which dims load on which axis for the current run, not from a fixed sign convention. PCA component signs are mathematically arbitrary; only the loading *pattern* is meaningful.
* **Per-group scores** for both PCs are written to `group_stats.tsv` (`EFI_PC1`, `EFI_PC2`).

This means EFI no longer claims to give a single scalar of "negative framing" per group. What it gives is the two largest axes of how groups differ in this corpus along the six framing dimensions. The corresponding research finding from the current corpus: bias surfaces along **two distinct modalities** — *erasure of personhood attribution* (PC1: dims AgI / PI / SI cluster opposite frame-netAttI / CEAT, captured by ~38% of variance) and *manner of personhood attribution within groups that receive it* (PC2: AgI vs. PI weighting among groups already attributed to, ~20% of variance). Both are legitimate readings of bias, and aggregating them into one scalar would erase the distinction.
...
2. **Calibrated Audit**: Once GTE-ModernBERT's thresholds were adjusted to match its embedding scale (`SEMANTIC_MIN = 0.62`, `SEMANTIC_MARGIN_MIN = 0.06`, `SEMANTIC_STRONG_MARGIN = 0.12`, `RESCUE_POS_MIN = 0.60`, `RESCUE_MARGIN_MIN = 0.06`), the True False Negative Rate dropped to **6.8%** (18/265).
