...
Foundational work on bias in language, predating or independent of LLM architectures as object of study. Entries are ordered by relevance to F3BF, with role/discourse overlap first and association-testing overlap second.
...
| 4 | [Mendelsohn & Budak (ACL 2025)](https://aclanthology.org/2025.acl-long.398/) "When People are Floods" | 400K US immigration tweets | Word-level + document-level LLM technique for seven predefined metaphor concepts; ideology × engagement analysis | **Analysis** | `metaphor-framing` `predefined-categories` `demographic-profiling` |
| 5 | [Caliskan, Bryson & Narayanan (2017)](https://doi.org/10.1126/science.aal4230) WEAT | Language corpora (Common Crawl via GloVe) | IAT analogue on word vectors; foundational proof that corpus-derived semantics encode human-like biases | **Detection** | `WEAT/SEAT-family` `predefined-categories` |
| 6 | [Azzalini, Dolci & Tanelli (2022)](https://www.semanticscholar.org/paper/Bias-Score%3A-Estimating-Gender-Bias-in-Sentence-Azzalini-Dolci/a8caf23b86b050ad217a05db6aac94396e73d37a) Bias Score | Sentence representations | Normalized per-word cosine similarity to a gender direction; word-importance weighting parallels SEAT context token weighting | **Detection** | `WEAT/SEAT-family` `sentence-level` |
...
- Mendelsohn 2020 (#3) and Mendelsohn & Budak 2025 (#4) form a `metaphor-framing` pair by the same first author: #3 studies news and LGBTQ discourse via w2v dehumanization clusters; #4 studies immigration tweets using LLMs as the measurement tool. Both are corpus-level analyses of human discourse bias with theory-derived categories and no association testing; neither studies LLMs as the object. The move of #4 here is justified by object of study (human political discourse, not LLM pretraining data), not by measurement tool (LLMs are used instrumentally, as Bamman used dependency parsers).
- `WEAT/SEAT-family` (#5–6): Caliskan is the foundational association-testing method F3BF inherits; Azzalini adds sentence-level weighting adjacent to SEAT context token weighting. 
...
Across 6 foundational + 3 model-internal + 6 output + 4 input + 3 all-stage + 4 bonus works, three converging gaps define F3BF's niche:
