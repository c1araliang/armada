...
|Phase 1 lexical-human rescue|`RESCUE_POS_MIN`, `RESCUE_MARGIN_MIN`|False rejects with demonym/ethnonym + human head, e.g. `American lady`, `Peruvian boys`, balanced against non-human lexical hits|
|Phase 1 corpus-noise review|reference/index/citation flags|Rows flagged `reference_noise_like:*` in `semantic_filter_review.tsv`|
...
|Phase 2 GTE frame refresh|`FRAME_SIM_FLOOR`, `FRAME_SIM_MARGIN`, anchor word list|Two-tier admission: sentence cosine for magnitude + anchor sentiment polarity for direction (no anchor floor). Verify on disputed candidates; consider POS-tag filtering to reject pure-evaluative adjectives that pass anchor (e.g. `right`, `high`); add metaphor handling as a separate diagnostic layer|
...
|Dimensional prototype matcher|`AGI_FLOOR`, `PI_FLOOR`, `SI_FLOOR`; per-dim independent gating; consider dual-gating/soft fusion|Verify on a held-out sample with multi-role sentences. Floor values are 70th-percentile empirical. Gating options include soft fusion (max + mean) to regularize against lexical/syntactic leakage; requires recalibrating floors to fit the new combined score distribution.|
...
**4. Address coordinate subject semantic bleeding (Known Issue)**

In short coordinate/parallel structures (e.g., "French and American people watched", "Black and White residents clashed"), sentence-level GTE ModernBERT embeddings of parallel mentions are virtually identical because they share the same clause context. This causes AgI, PI, and SI prototype similarity scores to co-fire uniformly across both mentions, collapsing individual semantic roles unless suppressed by direct syntactic filters (such as the SRL patient check for PI). Need to investigate word-level or attention-weighted target representation pooling to resolve this.

**5. Explore Dual-Gating / Soft Fusion for Dimensional Prototype Matcher**

Combine the current per-sentence max-pooling (which captures local syntactic fit but is prone to lexical/syntactic leakage) with a dimensional centroid mean similarity (which acts as a semantic regularizer). This helps reduce false positives from single-prototype word matches by ensuring the sentence overall aligns with the semantic direction of the target dimension. Requires recalibrating the empirical floors (`AGI_FLOOR`, `PI_FLOOR`, `SI_FLOOR`) to fit the new combined score distribution.
