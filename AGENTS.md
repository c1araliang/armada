...
- `X/run_pipeline.py`
  ...
  - Key sections:
    ...
    - `_refresh_frame_inventory()` — two-tier admission. Tier 1 (magnitude): `cos(candidate, sentence-level seed)` requiring `max(neg_sim, pos_sim) >= FRAME_SIM_FLOOR=0.55` and `|neg_sim - pos_sim| >= FRAME_SIM_MARGIN=0.06`. Tier 2 (direction & margin): `cos(candidate, sentiment anchor word)` must agree in sign with sentence direction (`anchor_diff * diff > 0`) AND clear directional anchor margins (`ANCHOR_NEG_MARGIN=0.01`, `ANCHOR_POS_MARGIN=0.025`). Writes `X/candidate_terms.json` with seed + auto + anchor fields preserved.
    ...
    - `_compute_efi()` — PCA on group × `[AGI, PI, SI, net_atti, weat, ceat]` matrix, where `net_atti` is frame-derived. Returns PC1 + PC2 loadings, scores, and variance-explained per component. Deterministic sign calibration locks PC2 so WEAT loading is positive (+0.637) and SI loading is negative (-0.626) across runs. **HIGH-RISK**.
